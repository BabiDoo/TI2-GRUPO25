...
if (btnCadastrar && window.location.pathname.includes('cadastro.html')) {
  btnCadastrar.addEventListener('click', () => {
    ...
    const confirmar = document.querySelector('input[placeholder="Confirmar senha"]').value.trim();
    const matricula = document.querySelector('input[placeholder="Instituição"]').value.trim();
  ...
  fetch('http://localhost:3000/api/projetos')
    ...
    .then(projetos => {
      if (projetos.length === 0) {
        listaProjetos.innerHTML = '<p>Nenhum projeto disponível no momento.</p>';
        return;
      }
